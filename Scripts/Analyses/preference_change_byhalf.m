%% Plot Linear Model Results - Split By First Half / Second Half
% Reviewer Response
...
half_of_session = {'First Half','Second Half'};
...
for half = 1:2
    ...
    for ii = 1:length(subjs)
            ...
            first_half = logical(Mchoice{ii}.trialnumber_bysession<227);
            % Create Table for Hill Trials
            temptable = table(subj,trialtype,trialnumber,first_half,choicetypecount,...
    ...
    %% HACK
    if half == 1
        TABLE = TABLE(TABLE.first_half==1,:);
    else
        TABLE = TABLE(TABLE.first_half==0,:);
    end    
    ...
    choice_table.first_or_second_half = logical(choice_table.first_or_second_half);
    
    %% HACK
    if half == 1
        choice_table = choice_table(choice_table.first_or_second_half == 1,:);
    else
        choice_table = choice_table(choice_table.first_or_second_half == 0,:);
    end        
